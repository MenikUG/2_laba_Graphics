...
namespace _2_laba_Graphics
{
	class Example_7
	{
		...
		public double RotateObject(double Angle, double Xoffset, double Yoffset, double Zoffset, double Pitch, double Yaw, double Roll, double x, double y, double z, double[] newX, double[] newY, int i)
		{
			...
			newX[i] = (Xoffset * Lu + Pitch * (-Yoffset * Yaw - Zoffset * Roll + Pitch * x + Yaw * y + Roll * z)
			+ (-Xoffset * Lu + Pitch * (Yoffset * Yaw + Zoffset * Roll - Yaw * y - Roll * z) + Lu * x) * Math.Cos(Angle) + SqrL * (-Zoffset * Yaw + Yoffset * Roll - Roll * y + Yaw * z) * Math.Sin(Angle)) / L;
			newY[i] = (Yoffset * Lv + Yaw * (-Xoffset * Pitch - Zoffset * Roll + Pitch * x + Yaw * y + Roll * z)
			+ (-Yoffset * Lv + Yaw * (Xoffset * Pitch + Zoffset * Roll - Pitch * x - Roll * z) + Lv * y) * Math.Cos(Angle) + SqrL * (-Zoffset * Pitch - Xoffset * Roll + Roll * x - Pitch * z) * Math.Sin(Angle)) / L;
			return (Zoffset * Lw + Roll * (-Xoffset * Pitch - Yoffset * Yaw + Pitch * x + Yaw * y + Roll * z)
			+ (-Zoffset * Lw + Roll * (Xoffset * Pitch + Yoffset * Yaw - Pitch * x - Yaw * y) + Lw * z) * Math.Cos(Angle) + SqrL * (-Yoffset * Pitch + Xoffset * Yaw - Yaw * x + Pitch * y) * Math.Sin(Angle)) / L; ;
		}
